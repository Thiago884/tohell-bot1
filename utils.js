{
  ...
  // ==============================================
  // NOVA FUNÇÃO: CHAR500 (1 ITEM POR PÁGINA)
  // ==============================================

  async function getChar500List(page = 1) {
    const perPage = 1; // MUDANÇA: 1 item por página para exibir a imagem grande (Userbar)
    const offset = (page - 1) * perPage;

    try {
      // Query com lógica filtrada:
      // 1. Deve ter 500 resets
      // 2. Se a guild for ToHeLL, ToHeLL2 ou ToHeLL10, DEVE ter status 'saiu' na tabela membros.
      // 3. Se for de outra guild (ou sem guild), mostra normalmente.
      
      const condition = `
        c.last_resets = 500
        AND (
          (c.guild IS NULL OR c.guild NOT IN ('ToHeLL_', 'ToHeLL2', 'ToHeLL10'))
          OR
          (c.guild IN ('ToHeLL_', 'ToHeLL2', 'ToHeLL10') AND m.status = 'saiu')
        )
      `;
